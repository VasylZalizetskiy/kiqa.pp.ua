...
public class AuthorizatioFilter implements Filter {
    ...
    private final AuthRule[] rules = {
    		...
    		new AuthRule("/messages/?", User.Permission.USER),  //in current project there is no page with such url 
            new AuthRule("/avatars/?", User.Permission.USER),
            new AuthRule("/moderator.*", User.Permission.MODERATE_POSTS) //in current project there is no page with such url
    }; 
